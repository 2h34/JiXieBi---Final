...
void DJmotor_Init(void)
{
    ...
    dj3508_param.CurrentLimit_raw = 16384;
    ...
    for (uint32_t i = 0; i < USE_DJNUM; i++)
    {
        PID_Init(&DJmotor[i].posPID, 0.4f, 0.13f, 0.07f, PIDPOS);
        PID_Init(&DJmotor[i].velPID, 7.0f, 0.4f, 0.0f, PIDINC);
    }
}
