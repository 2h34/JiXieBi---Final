...
static void Comm_CheckFinish(void)
{
    ...
    // && BlockVacuum_GetState() == BLOCK_VACUUM_RELEASED;
        ...
        case ARM_PICK_SIGNAL:   /* Pick：取块全流程走完 = pick DONE */
            done = (PickBlockTask_GetState() == PICK_BLOCK_TASK_DONE);
            break;

        case ARM_PLACE_SIGNAL:  /* Place: 机械臂移动到放块位置，进入微调状态 */
            done = (PlaceBlockTask_GetState() == PLACE_BLOCK_TASK_MANUAL_ALIGN);
            break;
        case ARM_RELEASE_SIGNAL: /* Release：放块全流程走完 = place DONE */
...
void BlockArmServiceTask(void *argument)
{
  ...
  for (;;)
  {
    ...
    /* 从命令 FIFO 取一条命令执行：出队即移除，保证不重复执行；
       无命令时返回 Master_Default，直接跳过 switch。 */
    uint8_t cmd = Comm_Dequeue();
    ...
    switch (cmd)
    {
      ...
      case Master_LowPick:
        ...
        s_finish_process = ARM_PICK_SIGNAL;
        break;
      ...
      case Master_PlaceBottom:
        ...
        s_finish_process = ARM_PLACE_SIGNAL;
        break;
      ...
      case Master_Release:
        ...
        s_finish_process = ARM_RELEASE_SIGNAL;
        break;
