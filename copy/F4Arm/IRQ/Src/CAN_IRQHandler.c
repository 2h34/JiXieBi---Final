...
/* 取块/放块/释放信号统一使用 arm_config.h 的 ARM_PICK_SIGNAL / ARM_PLACE_SIGNAL /
 * ARM_RELEASE_SIGNAL 宏，由编译宏决定本板响应 0x03/04/07 或 0x05/06/08。 */
...
/* 命令 FIFO：主控命令由 ISR(Master_message_handler) 与任务(Comm_ClickProcess)
 * 入队，BlockArmServiceTask 每 5ms 出队执行。相比原始的 volatile 单值缓冲，
 * 消除了"任务消费前第二条命令覆盖第一条命令"的丢帧窗口。
 * 8 深足够：命令是离散事件，比赛节奏下 5ms 窗口内最多 1~2 条。
 * 环形缓冲 + 关中断临界区，零动态内存，ISR 与任务上下文均可安全调用。 */
#define COMMAND_FIFO_SIZE 8U

static volatile uint8_t s_command_fifo[COMMAND_FIFO_SIZE];
static volatile uint8_t s_command_front = 0U; /* 出队侧游标 */
static volatile uint8_t s_command_rear = 0U;  /* 入队侧游标 */

void Comm_Enqueue(uint8_t cmd)
{
    __disable_irq();
    uint8_t next = (uint8_t)((s_command_rear + 1U) % COMMAND_FIFO_SIZE);
    if (next != s_command_front) /* 队列未满才写入，满则丢弃(命令低频,不该发生) */
    {
        s_command_fifo[s_command_rear] = cmd;
        s_command_rear = next;
    }
    __enable_irq();
}
...
{
    uint8_t cmd = Master_Default; /* 返回 0 表示无命令 */
    __disable_irq();
    if (s_command_front != s_command_rear)
    {
        cmd = s_command_fifo[s_command_front];
        s_command_front = (uint8_t)((s_command_front + 1U) % COMMAND_FIFO_SIZE);
    }
    __enable_irq();
    return cmd;
}
...
void Comm_ClickProcess(void)
{
    ...
    else if (signal == ARM_PLACE_SIGNAL)
    {
        if (count == 1U)
        {
...
// 接收关于右臂的消息
void Master_message_handler(CAN_HandleTypeDef *hcan, 
